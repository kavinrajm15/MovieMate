...
const ProfileRequests = () => {
  ...
  const canReview = canAccess('profile_requests', 'edit');

  const fetchRequests = async () => {
    try {
      const res = await fetch('http://localhost:5000/admin/profile_requests', {
        credentials: 'include',
  ...
  useEffect(() => {
    setFilteredRequests(requests.filter(r =>
      (r.current_admin_name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
      (r.current_theatre_name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const openReviewModal = (req) => { setCurrentReq(req); setIsModalOpen(true); };

  const handleAction = async (e, actionType) => {
    e.preventDefault();
    if (actionType === 'decline' && !window.confirm('Reject these profile changes?')) return;
    const formData = new FormData();
    formData.append('action', actionType);
    await fetch(`http://localhost:5000/admin/profile_requests/action/${currentReq.req_id}`, {
  ...
  if (status === 'pending')
    return `${base} bg-amber-500/15 text-amber-600 border-amber-500/30
      dark:bg-[rgba(245,197,24,0.1)] dark:text-[#F5C518] dark:border-[rgba(245,197,24,0.3)]`;
  // ── Red — declined or rejected ──
  return `${base} bg-rose-500/15 text-rose-600 border-rose-500/30
    dark:bg-[rgba(229,9,20,0.1)] dark:text-[#E50914] dark:border-[rgba(229,9,20,0.3)]`;
};
      ...
      <header className="flex justify-between items-center flex-wrap gap-4 mb-[30px]
        ...
        <div>
          ...
          <p className="text-[0.9rem] text-[#4a4e69] dark:text-[#B3B3B3]">
            Review account updates requested by Theatre Owners.
          </p>
        </div>

        <input
          type="text"
          placeholder="Search by owner, theatre, or type..."
          value={searchQuery}
        ...
        <>
          ...
          <div className="bg-white/40 dark:bg-[#1E1E1E]
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {['Theatre Owner', 'Update Type', 'Submitted On', 'Status'].map(h => (
                    <th key={h}
              ...
              <tbody>
                {currentItems.map((r, i) => (
                  <tr key={i} className="transition-colors duration-200
                    ...
                    <td className="px-[25px] py-[18px]
                      border-b border-white/30 dark:border-[#2A2A2A] align-middle">
                      <span className="px-3 py-[6px] rounded-full text-[0.85rem] font-bold
                        capitalize inline-block border text-center
                        bg-[#E3FCEF] text-[#38A169] border-transparent
      ...
      {isModalOpen && currentReq && (
        <div
          ...
          <div
            ...
            {/* Details box */}
            <div className="p-5 rounded-[12px] mb-5
              bg-slate-50 dark:bg-[#121212]
              border border-[#e2e8f0] dark:border-[#333333]">

              {currentReq.request_type === 'profile details' ? (
                /* ── Only show fields that actually changed ── */
                <div className="flex flex-col gap-4">
                  {[
                    ['Name',    currentReq.current_admin_name,    currentReq.new_name],
                    ['Phone',   currentReq.current_phone,         currentReq.new_phone],
                    ['Theatre', currentReq.current_theatre_name,  currentReq.new_theatre_name],
                    ['City',    currentReq.current_city,          currentReq.new_city],
                  ].filter(([, oldVal, newVal]) => oldVal !== newVal && newVal)
                  .map(([label, oldVal, newVal]) => (
                    <div key={label} className="flex flex-col gap-[5px]">
                      <strong className="text-[0.95rem] text-slate-800 dark:text-white">
                        {label}:
                      </strong>
                      <div className="flex items-center gap-4 px-4 py-[10px] rounded-lg
                        bg-white dark:bg-[#1a1a1a]
                        border border-[#e2e8f0] dark:border-[#333333]">
                        {/* Old value */}
                        <span className="flex-1 text-right line-through
                          text-[#94a3b8] dark:text-[#666666] font-medium">
                          {oldVal}
                        </span>
                        {/* Arrow */}
                        <span className="font-bold text-[#cbd5e1] dark:text-[#E50914]">
                          →
                        </span>
                        {/* New value */}
                        <span className="flex-1 font-bold
                          text-indigo-600 dark:text-[#F5C518]">
                          {newVal}
                        </span>
                      </div>
                    </div>
                ...
                <div className="text-center py-5">
                  <span className="text-[1.1rem] font-bold
                    text-slate-800 dark:text-white">
                    User has requested to change their account password.
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-4 mt-6">
              <button onClick={e => handleAction(e, 'approve')}
              ...
              <button onClick={e => handleAction(e, 'decline')}
